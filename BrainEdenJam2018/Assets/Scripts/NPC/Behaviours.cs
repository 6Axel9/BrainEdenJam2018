...
public class Steerings {
	...
	public static Vector3 Seek(Transform Origin, Transform Target)	{
        ...
        float deceleration = Mathf.Max(distance / 10, 0.15f);

        float new_speed = 2 * deceleration;

        Origin.rotation = Quaternion.Slerp(Origin.rotation, Quaternion.LookRotation(desired_direction), rotation_speed * Time.deltaTime);
    ...
    public static Vector3 Follow(Transform Origin, Transform Target)	{
		...
		desired_direction = (Target.TransformPoint(new Vector3(0.0f,0.0f,-2))) - Origin.position;
	...
	{
		float maxJitter = Random.Range(4, 8);
